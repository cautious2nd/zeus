 ...
 *	
 */
package org.scaffold.mybatis.generator.util.reflect;

import org.apache.commons.logging.Log;
import org.apache.commons.logging.LogFactory;
import org.scaffold.mybatis.generator.XMLReflectionIgnore;
import org.scaffold.mybatis.generator.util.date.MybatisDateUtil;
...
public class ReflectUtil {
	...
	public static List<Field> getFieldList(Class<?> clazz) {
		...
		for (Class<?> superClass = clazz; superClass != Object.class; superClass = superClass.getSuperclass()) {
			...
			for (Field field : fields) {
				if (field.getAnnotation(XMLReflectionIgnore.class) == null) {
					list.add(field);
				}
			}
		}
	...
	public static Object setValue(final Object obj, final String fieldName, final Object... values) {
		...
		if (field == null) {
			...
			method = getAccessibleMethod(obj, fieldName, MethodGen.METHOD_TYPE.get, os);
			if (method == null) {
				method = getAccessibleMethod(obj, fieldName, MethodGen.METHOD_TYPE.get, null);
			}
		...
		method = getAccessibleMethod(obj, fieldName, MethodGen.METHOD_TYPE.set, paramType);
		try {
		...
		if (String.class.equals(clazz)) {
			if (null == value || "".equals(value)) {
				...
				if (Date.class.equals(value.getClass())) {
					try {
						return MybatisDateUtil.getDate((Date) value, MybatisDateUtil.NdateFormatter.DATE_TIME);
					} catch (Exception ex) {
						ex.printStackTrace();
						return MybatisDateUtil.getDate((Date) value, MybatisDateUtil.NdateFormatter.DATE);
					}
				} else {
