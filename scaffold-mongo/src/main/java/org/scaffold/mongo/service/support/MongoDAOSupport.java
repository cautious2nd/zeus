/**
 * Author : chiziyue
 * Date : 2022年5月15日 下午8:48:12
 * Title : org.scaffold.mongo.service.support.MongoDAOSupport.java
 *
**/
package org.scaffold.mongo.service.support;
...
public interface MongoDAOSupport {
	...
	static final ScaffoldLogger SCAFFOLD_LOGGER = new ScaffoldLogger("mongodb ");
...
//					new UpdateOptions().upsert(isUpsert));
	...
	default <T> List<T> findManySort(MongoCollection<Document> collection, MongoFindFilter filter, Class<T> clazz) {
	...
	default <T> List<T> findManyByPage(MongoCollection<Document> collection, MongoFindFilter filter, Class<T> clazz) {
		...
		SCAFFOLD_LOGGER.debug(collection.getNamespace().getFullName() + "::findManyByPage::" + filter.filter().toString()
				+ "====" + result.size());
		return result;
	}
	...
	default long getCount(MongoCollection<Document> collection, MongoFindFilter filter) {
		return collection.countDocuments(filter.filter());
	}

	default long deleteMany(MongoCollection<Document> collection, MongoDeleteFilter filter) {
	...
	default List<Document> aggregate(MongoCollection<Document> collection, MongoAggregateFilter filter) {
		...
		SCAFFOLD_LOGGER.info(collection.getNamespace().getFullName() + ":aggregate:" + filter.info() + "====" + result.size());
